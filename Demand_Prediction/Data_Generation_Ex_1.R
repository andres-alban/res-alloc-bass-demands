...
p_coefs = rnorm(length(reg_cols), mean = 0, sd = 1) / as.numeric(colSums(clusters[,reg_cols]))/nrow(clusters)
...
r_coefs = rnorm(length(reg_cols), mean = 0, sd = 1) / as.numeric(colSums(clusters[,reg_cols]))/nrow(clusters)
...
# For the linear model, use variables 1,2, 3
...
for(i in 1:nrow(clusters)){
  ...
  if(runif(1) > 0.5){
    clusters[i,"training_visits"] = sample(max(1,min(round(inflection),60)):max(1,min(2*round(inflection),60)),1)
  } else{
