...
clusters = clusters[,colnames(clusters) %in% c("cluster_id","m_factor","beta","pop_size_b0","pop_size_b1","pop_size_b2","pop_size_b3",
                                               "Female","Ever_Married_Female_10_17","HH_Dist_Health_Gr_5km")]
...
p_coefs = rexp(length(reg_cols), rate=1) / as.numeric(colSums(clusters[,reg_cols]))/nrow(clusters)
p_coefs = (as.matrix(clusters[,reg_cols]) %*% p_coefs)[,1]
p_coefs = p_coefs - min(p_coefs)
true_parameters$p = p_coefs / max(p_coefs) * 5e-4 + 1e-4

# q is generated as a piecewise combination of two linear models. The combination is based on the first variable
q_coefs = rexp(length(reg_cols) - 1, rate=1) / as.numeric(colSums(clusters[,reg_cols[c(2,3)]]))/nrow(clusters)
q_coefs = (as.matrix(clusters[,reg_cols[1:(length(reg_cols)-1)]]) %*% q_coefs)[,1]
median_col = quantile(clusters[[reg_cols[1]]],0.5,na.rm=T)
q_coefs = q_coefs - min(q_coefs)
q_coefs = ifelse(clusters[[reg_cols[1]]] < median_col, q_coefs, q_coefs + mean(q_coefs))
true_parameters$q = q_coefs / max(q_coefs) * 1e-1 + 2e-2

# r is generated as a linear model
r_coefs = rexp(length(reg_cols), rate=1) / as.numeric(colSums(clusters[,reg_cols]))/nrow(clusters)
r_coefs = (as.matrix(clusters[,reg_cols]) %*% r_coefs)[,1]
r_coefs = r_coefs - min(r_coefs)
true_parameters$r = r_coefs / max(r_coefs) * 6/120 + 1/120
...
# c is generated as a linear model
# 1 values of c is shifted based on m -> this cannot be estimated and creates additional noise in the estimation process
c_coefs = rexp(length(reg_cols), rate=1) / as.numeric(colSums(clusters[,reg_cols]))/nrow(clusters)
c_coefs = (as.matrix(clusters[,reg_cols]) %*% c_coefs)[,1]
c_coefs = c_coefs - min(c_coefs)
true_parameters$c = c_coefs / max(c_coefs) * 400 + 100
...
# No site can have more than 70 visits (in effect, 47% have training visits above the inflection point)
...
for(i in 1:nrow(clusters)){
  ...
  if(runif(1) > 0.5){
    clusters[i,"training_visits"] = sample(max(1,min(round(inflection),70)):max(1,min(2*round(inflection),70)),1)
  } else{
    clusters[i,"training_visits"] = sample(1:min(round(inflection),70),1)
  }
  ...
  clusters[i,"ex_client_per_visit"] = runif(1,min=0,max=0.02) * c
...
save(clusters,visits,true_parameters,file="Data_Ex_2.RData")
