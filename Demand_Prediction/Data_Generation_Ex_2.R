...
p_coefs = rexp(length(reg_cols), rate=0.5)
p_coefs = (as.matrix(clusters[,reg_cols]) %*% p_coefs)[,1]
p_coefs = p_coefs - min(p_coefs)
true_parameters$p = p_coefs / max(p_coefs) * 5e-4 + 1e-4

# q is generated as a piecewise combination of two linear models
q_coefs = rexp(length(reg_cols) - 1, rate=0.5)
...
q_coefs = ifelse(clusters[[reg_cols[1]]] < median_col, q_coefs, q_coefs*3)
true_parameters$q = q_coefs / max(q_coefs) * 1e-1 + 2e-2

# r is generated as a linear model
r_coefs = rnorm(length(reg_cols), rate=0.5)
...
# 16 values of c are shifted based on m -> this cannot be estimated and creates additional noise in the estimation process
c_coefs = rnorm(length(reg_cols), rate=0.5)
...
# No site can have more than 70 visits (in effect, 44% have training visits above the inflection point)
